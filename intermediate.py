...
class Intermediate:
    ...
    def intrinsic_disorder_prediction (self):
        #Creates list of all proteins present in interaction data
        all_proteins =  pandas.concat([self.proteins_293T, self.proteins_HCT116, self.proteins_shared]).drop_duplicates()

        #Filters sequence_df on present proteins
        filt = self.sequence_df['ID'].isin(all_proteins)
        protein_sequences = self.sequence_df[filt]

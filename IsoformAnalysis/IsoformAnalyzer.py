...
class IsoformAnalyzer:
    def __init__ (self, ineraction_df, lib_df):
        self.interaction_df = ineraction_df
        self.lib_df = lib_df
        
        self.unique_isoforms = self._read_csv (self.interaction_df)
        
        self.iso_lib_match = self.iso_lib_matcher(self.unique_isoforms, self.lib_df)
        self.can_lib_match = self.can_lib_matcher(self.unique_isoforms, self.lib_df)
        self.gene_lib_match = self.gene_lib_matcher( self.unique_isoforms, self.lib_df)
        
        self.lib_match = self.addition(self.iso_lib_match, self.can_lib_match, self.gene_lib_match, 'isoform')
        self.undefined, self.undefined_sorted = self.subtraction(self.unique_isoforms, self.lib_match, 'isoform', 'isoform')
        self.index = self.addition(self.lib_match, self.undefined_sorted, None, 'isoform')
        
        self.proteins = self.isoform_count(self.index)
       
    def isoform_distribution (self):
        df = self.proteins
    ...
    def _read_csv(self, data_df, check_duplicates = False):
        ...
        if check_duplicates == True:
            self.check_duplicates(unique_isoforms_df)

        return unique_isoforms_df

    def iso_lib_matcher (self, dataset_df, lib_df, check_duplicates = False):
        ...
        if check_duplicates == True: 
            self.check_duplicates(filtered_df)

        return filtered_df
        
    def can_lib_matcher(self, dataset_df, lib_df, check_duplicates = False):
    ...
    def gene_lib_matcher (self, dataset_df, lib_df, check_duplicates = False):
    ...
    def addition(self, dataset_1, dataset_2, dataset_3, column_drop_duplicates, check_duplicates = False):
        ...
        if check_duplicates == True: 
            self.check_duplicates(merged_df)
            
        return merged_df
    
    def subtraction (self, add_df, subtract_df, add_column, subtract_column):
    ...
    def isoform_count(self, dataset, print_total = False):
        isoform_dict = {}
